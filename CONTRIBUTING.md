...
- **推送失败不阻塞执行（批次二十二用户拍板）**：推送不成功时 SHALL NOT 阻塞当前工作——先本地 commit（本地照常推进），**等到下一个 commit 完成时再一起推送到远程**，以此类推（累积推送）；本地堆积的 commit 在下次推送时全部带上。
- **每个 commit 后尝试推送**——若成功则保持同步；若失败则继续本地工作，不等待。
- **推送失败后的重试**：`git push` 失败后先确认本地状态（`git status`），继续工作；下次 commit 时自然带上未推送的 commit 一起推。
- **不重写历史**：未推送的 commit 不 squash/amend（保持批次粒度），推送时按序推送即可。

> **本会话备注**：GitHub 网络多次偶发超时（130+ 秒），经重试或累积推送成功。这是 GitHub 端偶发问题，不是配置错误。
